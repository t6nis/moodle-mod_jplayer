...
/**
 * Prints a particular instance of jplayer
 *
...
if ($id) {
    $cm         = get_coursemodule_from_id('jplayer', $id, 0, false, MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $jplayer  = $DB->get_record('jplayer', array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
    $jplayer  = $DB->get_record('jplayer', array('id' => $n), '*', MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $jplayer->course), '*', MUST_EXIST);
    $cm         = get_coursemodule_from_instance('jplayer', $jplayer->id, $course->id, false, MUST_EXIST);
} else {
...
$PAGE->set_title(format_string($jplayer->name));
...
if ($jplayer->intro) { // Conditions to show the intro can change to look for own settings or whatever
